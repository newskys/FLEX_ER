...
const WidgetMoreButton = ({ userSchedule, notices, isNarrow }: Props) => {
  ...
  const setModalStore = useSetRecoilState($modalStore)
  ...
  useEffect(() => {
    setModalStore({
      isOpened: !!modalData || showSettingsModal,
    })
  }, [modalData, showSettingsModal])
      ...
      {isMenuOpened && moreButtonRef.current && (
        <>
          <div
            ...
            className={`z-[2147483647] absolute mt-2 w-[135px] rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 focus:outline-none`}
            role="menu"
